...
% ARG 2: X pos
% ARG 3: text
% ARG 4: rotation angle (degrees)
\newcommand{\sextupole}[4][0.0]{%
  \begin{scope}[shift={(#2,#1)}]             % move to (X,Y)
    \begin{scope}[rotate=#4, yscale=2.4] % align with beam, then stretch in Y
      \foreach \ang in {0,60,120}{%
        \begin{scope}[rotate=\ang]
          \filldraw[cyan] (-0.25,-0.08) rectangle (0.25,0.08);
        \end{scope}
      }
    \end{scope}
  \end{scope}
...
\newcommand{\prescatt}[3][0.0]{
    \draw[ultra thick, brown] (#2,#1) circle (0.3);
    \draw[ultra thick, brown] (#2-0.15,#1-0.15) -- (#2+0.15,#1+0.15);
    \draw[ultra thick, brown] (#2-0.15,#1+0.15) -- (#2+0.15,#1-0.15);
...
\newcommand{\scatt}[3][0.0]{
    \def\radius{0.3}
    \def\trianglesize{0.18}
    \draw[ultra thick, brown] (#2,#1) circle (\radius);

    % Triangle with tip pointing RIGHT
    \draw[ultra thick, brown] 
        (#2+\trianglesize,#1) --  % right point (tip)
        (#2-\trianglesize*0.5,#1+\trianglesize*0.866) -- % top-left
        (#2-\trianglesize*0.5,#1-\trianglesize*0.866) -- % bottom-left
        cycle;
